...
5. **Row key matches → unlink; only on `removed === true`** log the removal and DM. An unlink failure logs an error and does NOT consume the notification guard. A DM-send failure after a successful unlink also releases the guard, but since the row is already gone by then, no later call for that session key can re-enter this path: the release is a defensive no-op under the current call graph, not an active retry. Delivery is best-effort, at-most-once per dead session key, not guaranteed-once.
