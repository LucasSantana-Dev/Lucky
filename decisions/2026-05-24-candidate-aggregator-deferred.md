...
- The `AutoplayContext` value object (ADR `decisions/2026-05-23-autoplay-context-value-object.md`) and the `recommendTracks()` single entrypoint (ADR `decisions/2026-05-23-recommendation-engine-single-entrypoint.md`) already reduced the surface area that a future CandidateAggregator seam would need to cross.
...
- `decisions/2026-05-23-autoplay-context-value-object.md` — AutoplayContext VO (type in place, builder pending).
- `decisions/2026-05-23-recommendation-engine-single-entrypoint.md` — `recommendTracks()` single entrypoint (implemented).
- `decisions/2026-05-19-guild-automation-module-executors.md` — Guild Automation executor extraction (prerequisite for revisiting this ADR).
