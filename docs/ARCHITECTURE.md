...
| **shared**   | No process entry                        | Consumed by bot and backend via `@lucky/shared`.                              |
...
- **Database**: PostgreSQL with Prisma (schema in repo root; shared client via `@lucky/shared`). Supabase is not used for the bot or backend; Prisma + Postgres keeps one source of truth and avoids splitting data between Supabase and the bot. Supabase can be considered later only if a separate product (e.g. a Next.js app with its own auth/RLS) needs it.
...
- **bot**: Depends on `@lucky/shared` only. Commands and music logic live in `packages/bot`; add new commands there.
- **backend**: Depends on `@lucky/shared` only. API routes and auth live in `packages/backend`; add new API routes there.
...
- **prisma/** – Schema and migrations (single source of truth; shared via `@lucky/shared`).
...
| **bot**      | `src/functions/{general,music}/commands/`, `src/handlers/`, `src/utils/`, `src/services/`, `src/events/` | New slash command: add file under `functions/<category>/commands/` or folder + re-export (see command loading below). Handlers, utils, and services by domain. |
...
### Internal layer boundaries (bot package)

`packages/bot/src/` has two boundaries that are documented conventions, not lint-enforced rules (decision below):

- **`handlers/` vs `functions/`** — `handlers/` is global bot lifecycle infra (player events, button/context-menu routing); `functions/` is feature command code, organized by category (`music/`, `moderation/`, etc.). The intended direction is `functions/` may call into `handlers/`, not the reverse — a handler routing to feature code (e.g. `handlers/musicButtonHandler.ts` calling into `functions/music/...`) is the layer inversion to avoid. An architecture audit (2026-08-10) found this already violated in both directions; tracked in [#1961](https://github.com/LucasSantana-Dev/Lucky/issues/1961).
- **`utils/` vs `services/`** — `utils/` is for pure, stateless helper functions. Anything holding mutable state, running a timer/interval, or making a DB/external-API call is a service and belongs in `services/`, not `utils/`. The same audit found stateful classes (e.g. `MusicWatchdogService` in `utils/music/watchdog.ts`, `BirthdayScheduler` in `utils/general/birthdayScheduler.ts`, autoplay orchestration with Spotify calls in `utils/music/autoplay/`) that read as utilities but aren't; tracked in [#1968](https://github.com/LucasSantana-Dev/Lucky/issues/1968).

**Current decision (2026-08-10, after researching module-boundary tooling and debating the tradeoffs — see #1961 and #1968 for the discussion):** don't add a lint dependency for this yet. Both violations are acyclic (CI's `madge` cycle check doesn't catch them) and not currently blocking development. Adding enforcement tooling _while_ onboarding external contributors was judged higher-friction than the debt itself at this repo's change velocity. Plan, in order:

1. Document the rule here (this section) + gate the two boundaries via `.github/CODEOWNERS` as an interim, review-based check.
2. Physically move the stateful code flagged in #1968 out of `utils/` into `services/` (no tooling needed for this step).
3. After external contributors have been onboarded for a while, check whether either boundary is actually getting violated in practice.
4. Only if it is: add an ESLint `no-restricted-imports` rule for the specific violated direction, as a warning first. A dedicated architecture-linter package was evaluated and rejected — see #1961 for the reasoning.

If you're touching either boundary and unsure which side something belongs on, ask in the PR rather than guessing — that's cheaper than the tooling for now.

### Repo checklist (matches this doc)

- No `src/` at repo root; all runtime code under `packages/`.
- Prisma schema and migrations in `prisma/` at repo root; client used via `@lucky/shared`.
...
- **Prisma**: Schema and migrations in `prisma/` at repo root; client from `@lucky/shared`. Used for guild settings, track history, feature toggles, and app data.
