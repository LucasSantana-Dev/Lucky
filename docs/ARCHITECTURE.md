...
- **`utils/` vs `services/`** — `utils/` is for pure, stateless helper functions. Anything holding mutable state, running a timer/interval, or making a DB/external-API call is a service and belongs in `services/`, not `utils/`. The same audit found `utils/music/` (136 files) full of stateful classes (`MusicWatchdogService`, `BirthdayScheduler`, autoplay orchestration with Spotify calls) that read as utilities but aren't; tracked in [#1968](https://github.com/LucasSantana-Dev/Lucky/issues/1968).
...
1. Document the rule here (this section) + gate the two directories via `.github/CODEOWNERS` as an interim, review-based check.
