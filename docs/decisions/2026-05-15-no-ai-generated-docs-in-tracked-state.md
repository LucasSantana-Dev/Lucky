...
4. **Policy** for what may live in the tracked docs tree:
    - **Allowed:** `docs/decisions/` (ADRs), `docs/runbooks/`, `docs/specs/`, human-authored operational/architectural references with stated owners (README, ARCHITECTURE, CI_CD, \*\_SETUP, etc.), brand canon tied to actual code (BRANDING_GUIDE, DESIGN_SYSTEM).
    - **Not allowed:** RAG context dumps, AI-tool prompts, generated reference compendia that duplicate upstream docs, planning docs that aren't ADRs, snapshot status reports.
