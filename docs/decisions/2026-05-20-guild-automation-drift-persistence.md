...
- Parent ADR `docs/decisions/2026-05-19-guild-automation-module-executors.md` claimed "Drift detection is automatic: Capture + Diff produces a Drift result that the orchestrator can persist." That promise is unmet at PR #906.
- Sibling ADR `docs/decisions/2026-05-20-guild-automation-executor-partial-failure.md` introduces `ExecutorApplyResult<T> = { status: 'success' | 'partial' | 'failed' }`. The reader of that union is the natural place to update drift, because it already knows per-module success / partial / failure.
...
- `docs/decisions/2026-05-19-guild-automation-module-executors.md` — parent ADR; this fulfills the "persist drift automatically" promise.
- `docs/decisions/2026-05-20-guild-automation-executor-composition.md` — sibling ADR (composition root); shares the orchestrator reader path.
- `docs/decisions/2026-05-20-guild-automation-executor-partial-failure.md` — sibling ADR (apply-result shape); the `ExecutorApplyResult` it introduces is the input this ADR's reader consumes.
