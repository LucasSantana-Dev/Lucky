...
export async function addTracksToQueue(
    ...
    try {
        ...
        const { validTracks, invalidTracks } = validateTracks(
            tracks,
            queue,
            managementOptions,
        )
        ...
        const tracksToAdd = validTracks.slice(
            0,
            Math.min(availableSlots, options.maxTracks || validTracks.length),
        )
        ...
        const tracksSkipped =
            tracksToAdd.length - tracksAdded + invalidTracks.length
        const message =
            tracksAdded > 0
