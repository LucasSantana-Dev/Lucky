...
function checkForDuplicates(
    ...
    for (const queueTrack of queue.tracks.toArray()) {
        if (calculateTrackSimilarity(track, queueTrack) >= threshold)
            return true
    }
    return false
}
...
export function validateTrack(
    ...
    try {
        if (!track || !track.title || !track.url)
            return { isValid: false, reason: 'Invalid track data' }
        ...
        if (duration > 600000)
            return { isValid: false, reason: 'Track too long (max 10 minutes)' }
        if (duration < 30000)
            return {
                isValid: false,
