...
jest.mock('../../misc/stringUtils', () => ({
	...
	calculateSimilarity: (...args: unknown[]) => calculateSimilarityMock(...args),
...
describe('TitleComparisonService', () => {
	...
	beforeEach(() => {
		...
		normalizeStringMock.mockImplementation((str: string) => str.replace(/\s+/g, ' ').trim())
	})
	...
	describe('constructor', () => {
		it('should initialize with default options', () => {
			...
			expect(safeSetIntervalMock).toHaveBeenCalledWith(expect.any(Function), 600000)
		})
		...
		it('should start cache cleanup interval on initialization', () => {
			expect(safeSetIntervalMock).toHaveBeenCalledTimes(1)
			expect(safeSetIntervalMock).toHaveBeenCalledWith(expect.any(Function), 600000)
		})
	})
	...
	describe('extractArtistTitle', () => {
		it('should extract artist and title from input', () => {
			applyPatternsMock.mockReturnValue({ artist: 'The Beatles', title: 'Hey Jude' })
		...
		it('should trim whitespace from extracted artist and title', () => {
			applyPatternsMock.mockReturnValue({ artist: '  Artist  ', title: '  Title  ' })
		...
		it('should fallback to Unknown artist when only title is extracted', () => {
			applyPatternsMock.mockReturnValue({ artist: '', title: 'Song Title' })
	...
	describe('isSimilarTitle', () => {
		...
		it('should normalize titles before comparison', () => {
			normalizeStringMock.mockImplementation((str: string) => str.toLowerCase().trim())
			calculateSimilarityMock.mockReturnValue(0.9)
			service.isSimilarTitle('  Song  ', '  SONG  ')
			expect(normalizeStringMock).toHaveBeenCalledTimes(2)
		})

		it('should handle case insensitive comparison by default', () => {
			normalizeStringMock.mockImplementation((str: string) => str.toLowerCase())
			calculateSimilarityMock.mockReturnValue(0.85)
			const result = service.isSimilarTitle('HEY JUDE', 'hey jude')
			expect(result).toBe(true)
			expect(normalizeStringMock).toHaveBeenCalledTimes(2)
		...
		it('should compare identical titles as similar', () => {
			...
			const result = service.isSimilarTitle('Perfect Match', 'Perfect Match')
			expect(result).toBe(true)
		})
	})
	...
	describe('calculateSimilarity', () => {
		...
		it('should normalize both titles before calculating', () => {
			normalizeStringMock.mockImplementation((str: string) => str.toLowerCase())
			calculateSimilarityMock.mockReturnValue(0.85)
			service.calculateSimilarity('TITLE', 'title')
			expect(normalizeStringMock).toHaveBeenCalledTimes(2)
		})
	})
	...
	describe('cache management', () => {
		...
		it('should log when cache is cleared by cleanup', () => {
			...
			const svc = new TitleComparisonService()
			const cleanupCallback = safeSetIntervalMock.mock.calls[0][0] as () => void
			cleanupCallback()
	...
	describe('title normalization', () => {
		it('should lowercase titles when case insensitive (default)', () => {
			normalizeStringMock.mockImplementation((str: string) => str.toLowerCase())
			calculateSimilarityMock.mockReturnValue(0.85)
			service.isSimilarTitle('HEY JUDE', 'hey jude')
			expect(normalizeStringMock).toHaveBeenCalledTimes(2)
		})

		it('should respect case sensitivity option', () => {
			const svc = new TitleComparisonService({ caseSensitive: true })
			normalizeStringMock.mockImplementation((str: string) => str)
			calculateSimilarityMock.mockReturnValue(0.85)
			svc.isSimilarTitle('HEY JUDE', 'hey jude')
			// normalizeString should still be called for whitespace normalization
			expect(normalizeStringMock).toHaveBeenCalled()
		})

		it('should normalize whitespace when flag is true (default)', () => {
			normalizeStringMock.mockImplementation((str: string) => str.replace(/\s+/g, ' ').trim())
			calculateSimilarityMock.mockReturnValue(0.85)
			service.isSimilarTitle('Hey  Jude', 'Hey Jude')
			expect(normalizeStringMock).toHaveBeenCalledTimes(2)
		...
		it('should skip whitespace normalization when flag is false', () => {
			const svc = new TitleComparisonService({ normalizeWhitespace: false })
			normalizeStringMock.mockImplementation((str: string) => str)
			calculateSimilarityMock.mockReturnValue(0.85)
			svc.calculateSimilarity('Hey  Jude', 'Hey Jude')
			// normalizeString should not be called
			expect(normalizeStringMock).not.toHaveBeenCalled()
		})
	})

	describe('integration scenarios', () => {
		it('should handle full workflow: extract, compare, get cache size', () => {
			applyPatternsMock.mockReturnValue({ artist: 'Beatles', title: 'Hey Jude' })
			normalizeStringMock.mockImplementation((str: string) => str.toLowerCase().trim())
			calculateSimilarityMock.mockReturnValue(0.9)

			const extracted = service.extractArtistTitle('The Beatles - Hey Jude')
			...
			const similarity = service.calculateSimilarity('Hey Jude', 'hey jude')
	...
	describe('edge cases', () => {
		...
		it('should handle very long input strings', () => {
			...
			applyPatternsMock.mockReturnValue({ artist: 'Artist', title: longString })
		...
		it('should handle special characters in titles', () => {
			...
			applyPatternsMock.mockReturnValue({ artist: 'Journey', title: specialTitle })
		...
		it('should handle unicode characters', () => {
			...
			applyPatternsMock.mockReturnValue({ artist: 'Artist', title: unicodeTitle })
		...
		it('should handle emoji in titles', () => {
			...
			applyPatternsMock.mockReturnValue({ artist: 'Artist', title: emojiTitle })
