...
function parseDuration(videoInfo: unknown): number {
    return parseInt(
        (videoInfo as { videoDetails?: { lengthSeconds?: string } })
...
function validateStream(stream: unknown): stream is { stream: Readable } {
    return (
        ...
        'stream' in (stream as Record<string, unknown>)
    )
}
...
export class DownloadVideoService {
    ...
    private async downloadAudio(
        ...
        try {
            ...
            if (!validateStream(stream))
                return { success: false, error: 'Could not get audio stream' }

            const result = await convertStreamToFile({
                input: stream.stream as Readable,
                output: audioPath,
            ...
            return {
                ...
                fileSize: stats.size,
                duration: parseDuration(videoInfo),
            }
        } catch (error) {
            return {
                success: false,
    ...
    private async downloadVideoFile(
        ...
        try {
            const stream = await play.stream(url, { quality: 0 })
            if (!validateStream(stream))
                return { success: false, error: 'Could not get video stream' }
