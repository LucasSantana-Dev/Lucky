...
describe('ticket-setup command', () => {
    ...
    describe('set subcommand', () => {
        ...
        it('rejects @everyone as the agent role', async () => {
            const interaction = createInteraction('set', {
                options: {
            ...
            await ticketSetupCommand.execute({ client: {}, interaction } as any)
            expect(setGuildSettingsMock).not.toHaveBeenCalled()
        ...
        it('rejects managed integration roles', async () => {
            const interaction = createInteraction('set', {
                options: {
                    getSubcommand: jest.fn().mockReturnValue('set'),
                    getChannel: jest
        ...
        it('rejects setup when the bot lacks Manage Channels in the category', async () => {
            const interaction = createInteraction('set', {
                guild: {
                    members: {
                        me: {
