            ...
            sub
                ...
                .addStringOption((o) =>
                    o
                        .setName('message_id')
                        .setDescription(
                            'Message ID of the reaction role message',
                        )
                        .setRequired(true),
                ),
        )
    ...
    execute: async ({ interaction }) => {
        ...
        try {
            if (subcommand === 'create')
                await handleCreate(interaction, interaction.guild)
            else if (subcommand === 'delete')
                await handleDelete(interaction, interaction.guild)
            else if (subcommand === 'list')
                await handleList(interaction, interaction.guild)
        } catch (error) {
