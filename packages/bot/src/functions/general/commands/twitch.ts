    ...
    data: new SlashCommandBuilder()
        ...
        .addSubcommand((sub) =>
            sub
                .setName('add')
                .setDescription(
                    'Notify this channel when a Twitch streamer goes live',
                )
                .addStringOption((opt) =>
                    opt
                        .setName('username')
                ...
                .addChannelOption((opt) =>
                    opt
                        .setName('channel')
                        .setDescription(
                            'Discord channel to notify (default: this channel)',
                        ),
                ),
        )
        .addSubcommand((sub) =>
        ...
        .addSubcommand((sub) =>
            sub
                .setName('list')
                .setDescription('List Twitch streamers you get notified for'),
        )
    ...
    execute: async ({ interaction }) => {
        ...
        try {
            ...
            if (subcommand === 'remove')
                return await handleTwitchRemove(interaction)
            if (subcommand === 'list')
                return await handleTwitchList(interaction)
        } catch (err) {
