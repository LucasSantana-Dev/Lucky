...
export async function handleSetExclusive(
    interaction: ChatInputCommandInteraction,
): Promise<void> {
...
export async function handleRemoveExclusive(
    interaction: ChatInputCommandInteraction,
): Promise<void> {
    const role = interaction.options.getRole('role', true)
    const excludedRole = interaction.options.getRole('excluded_role', true)
...
export async function handleListExclusive(
    interaction: ChatInputCommandInteraction,
): Promise<void> {
    ...
    const embed = new EmbedBuilder()
        ...
        .setDescription(
            (exclusions as Array<{ roleId: string; excludedRoleId: string }>)
                .map(
                    (ex, i: number) =>
                        `${i + 1}. <@&${ex.roleId}> excludes <@&${ex.excludedRoleId}>`,
                )
                .join('\n'),
        )
        .setTimestamp()
