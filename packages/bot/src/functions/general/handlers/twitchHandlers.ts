...
import { getPrismaClient } from '@nexus/shared/utils'
...
export async function handleTwitchAdd(
    interaction: ChatInputCommandInteraction,
): Promise<void> {
    ...
    if (success) {
        ...
        await replyError(
            interaction,
            'Error',
            'Failed to add Twitch notification.',
        )
    }
}

export async function handleTwitchRemove(
    interaction: ChatInputCommandInteraction,
): Promise<void> {
    const username = interaction.options
        .getString('username', true)
    ...
    if (!guild) {
        await replyError(
            interaction,
            'Not found',
            'No Twitch notification for that user in this server.',
        )
        return
    }
...
export async function handleTwitchList(
    interaction: ChatInputCommandInteraction,
): Promise<void> {
    const prisma = getPrismaClient()
    const guild = await prisma.guild.findUnique({
    ...
    if (!guild) {
        await replySuccess(
            ...
            'No Twitch streamers configured for this server.',
        )
        return
    }
