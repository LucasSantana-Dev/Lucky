...
export default new Command({
    data: new SlashCommandBuilder()
        ...
        .setDescription('Remove a role from every non-bot member holding a given role')
        .setDefaultMemberPermissions(PermissionFlagsBits.ManageRoles)
        .addRoleOption((option) =>
            option
                .setName('filter_role')
                .setDescription('Members with this role will lose the target role')
                .setRequired(true),
        )
        .addRoleOption((option) =>
            option
                .setName('target_role')
                .setDescription('The role to remove from each member')
                .setRequired(true),
        )
    ...
    execute: async ({
        ...
        if (dryRun) {
            await interaction.editReply({
                content: `📊 **Dry Run**: **${totalEstimate}** member(s) with <@&${filterRole.id}> would lose <@&${targetRole.id}>.\n\nRun again without \`dry_run\` to proceed.`,
            })
            return
        }

        const { BulkRemoveRoleExecutor } = await import('../batch/bulkRemoveRoleExecutor')
        ...
        const confirmed = await showBatchConfirmation(interaction, {
            operation: 'bulk remove role',
            totalItems: totalEstimate,
            estimatedMinutes,
            fidelityWarnings: [
                `All ${totalEstimate} non-bot member(s) will lose <@&${targetRole.id}>`,
                'The server owner and members above the bot are skipped',
                'Members who do not have the target role are skipped',
            ],
        })
        ...
        try {
            ...
            if (!enqueueResult) {
                await interaction.editReply({
                    content: '❌ Failed to queue bulk remove-role job (Redis unavailable). Please try again.',
                })
                return
            }

            await interaction.editReply({
                content: `✅ Bulk remove-role queued (ID: \`${job.id}\`) — ${totalEstimate} member(s). Track progress on the **Batch Jobs** dashboard page.`,
            })

            infoLog({
                message: `Bulk remove-role job created: ${job.id}`,
                data: {
                    guildId: guild.id,
                    filterRoleId: filterRole.id,
                    targetRoleId: targetRole.id,
            ...
            errorLog({ message: 'Failed to create bulk remove-role job', error })
            await interaction.editReply({
