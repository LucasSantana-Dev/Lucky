...
export default new Command({
    data: new SlashCommandBuilder()
        ...
        .addSubcommand((sub) =>
            sub
                .setName('delete')
                .setDescription('Delete a case (requires Administrator)')
                .addIntegerOption((opt) =>
                    opt
                        .setName('case_number')
    ...
    execute: async ({ interaction }) => {
        ...
        try {
            if (subcommand === 'view')
                return await handleCaseView(interaction, caseNumber)
            if (subcommand === 'update')
                return await handleCaseUpdate(interaction, caseNumber)
            if (subcommand === 'delete')
                return await handleCaseDelete(interaction, caseNumber)
        } catch (error) {
            errorLog({
                message: `Failed to ${subcommand} case`,
                error: error as Error,
            })
            await interactionReply({
                interaction,
                content: {
                    content: `❌ Failed to ${subcommand} case. Please try again.`,
                },
            })
        }
    },
