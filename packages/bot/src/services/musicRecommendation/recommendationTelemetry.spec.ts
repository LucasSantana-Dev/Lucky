...
describe('recommendationTelemetry', () => {
	...
	describe('recordRecommendationPick', () => {
		...
		it('stores signals array directly in JSON column', async () => {
			const input: RecordPickInput = {
				...
				basis: {
					source: 'genre-tag',
					signals: ['energy match', 'session novelty', 'genre family drift'],
				},
			}
		...
		it('DB error is swallowed and resolved void', async () => {
			...
			// Should not throw
			await expect(recordRecommendationPick(input)).resolves.toBeUndefined()
	...
	describe('recordRecommendationOutcome', () => {
		...
		it('DB error is swallowed and resolved void', async () => {
			...
			// Should not throw
			await expect(recordRecommendationOutcome(args)).resolves.toBeUndefined()
			expect(mockErrorLog).toHaveBeenCalled()
		})
	})
})
